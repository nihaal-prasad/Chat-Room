...
public class Encryptor {
    ...
    public void generatePublicKey(String encodedServerKey) {
        try {
            // Generate the server's public key from the received encoded key
            // KeyFactory keyFactory = KeyFactory.getInstance("EC");
            // X509EncodedKeySpec keySpec = new
            // X509EncodedKeySpec(Base64.getDecoder().decode(encodedServerKey));
            // PublicKey serverPublicKey = keyFactory.generatePublic(keySpec);

            // Generate the DH parameter specifications from the received key
            // DHParameterSpec dhParam = ((DHPublicKey) serverPublicKey).getParams();
